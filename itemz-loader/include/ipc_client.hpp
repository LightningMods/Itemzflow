...
int OpenConnection(const char* path) {
    ...
    if (soc == -1) {
        logshit("Failed to create socket");
        return -1;
    }
    ...
    if (connect(soc, (struct sockaddr*)&server, SUN_LEN(&server)) == -1) {
        close(soc);
        logshit("Failed to connect to socket");
        return -1;
    }
    return soc;
}
...
int IPCSendData(const IPCMessage& msg) {
    ...
    if (ret < 0) {
        logshit("IPCSendData failed with: %s", strerror(errno));
    }
    return ret;
}
...
IPC_Ret IPCSendCommand(IPC_Commands cmd, std::string& ipc_msg, bool silent = false) {
    ...
    if (IPCOpenIfNotConnected()) {
        ...
        if(!silent)
            logshit(" Sent IPC Command %i", static_cast<int>(cmd));
        ...
        if (static_cast<int>(readSize) >= 0) {
            ...
            if (error != IPC_Ret::INVALID) {
                ...
                if(!silent)
                   logshit("[ItemzDaemon] Daemon IPC Response: %s, code: %s, readSize: %i", msg.msg, error == IPC_Ret::NO_ERROR ? "NO_ERROR" : "Other", readSize);
            } else {
...
IPC_Ret IPCMountFUSE(std::string ip, std::string path, bool debug_mode) {
    ...
    if (debug_mode) {
        if (IPCSendCommand(IPC_Commands::FUSE_SET_DEBUG_FLAG, in) == IPC_Ret::NO_ERROR) {
            logshit("FUSE debug mode enabled");
        } else {
            logshit("FUSE failed to enable debug mode");
        }
    }
    ...
    if ((error = IPCSendCommand(IPC_Commands::FUSE_START_W_PATH, path)) == IPC_Ret::NO_ERROR) {
        logshit("FUSE started");
    } else {
        logshit("FUSE failed to start with error code: %i", static_cast<int>(error));
    }
