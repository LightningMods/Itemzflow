					...
					const struct cuse_lowlevel_ops *clop)
{
	...
	if (dev_info_len > CUSE_INIT_INFO_MAX) {
		libfuse_print("cuse: dev_info (%zu) too large, limit=%u",
	...
	if (!cd) {
		libfuse_print("cuse: failed to allocate cuse_data");
		return NULL;
	}
...
void cuse_lowlevel_init(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	(void) nodeid;
	if (f->debug) {
		libfuse_print("CUSE_INIT: %u.%u", arg->major, arg->minor);
	...
	if (f->debug) {
		libfuse_print("   CUSE_INIT: %u.%u",
			outarg.major, outarg.minor);
		libfuse_print("   flags=0x%08x", outarg.flags);
		libfuse_print("   max_read=0x%08x", outarg.max_read);
		libfuse_print("   max_write=0x%08x", outarg.max_write);
		libfuse_print("   dev_major=%u", outarg.dev_major);
		libfuse_print("   dev_minor=%u", outarg.dev_minor);
		libfuse_print("   dev_info: %.*s", cd->dev_info_len,
			cd->dev_info);
	}
					 ...
					 int *multithreaded, void *userdata)
{
	...
	if (fd == -1) {
		if (errno == ENODEV || errno == ENOENT)
			libfuse_print("fuse: device not found, try 'modprobe cuse' first");
		else
			libfuse_print("fuse: failed to open %s: %s",
