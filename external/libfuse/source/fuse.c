...
static int fuse_load_so_name(const char *soname)
{
	...
	if (so->handle < 0) {
		libfuse_print("fuse: %s", dlerror());
		goto err;
	}
	if (!so->ctr) {
		libfuse_print("fuse: %s did not register any modules",
			soname);
		goto err;
	}
	return 0;
...
static int fuse_load_so_module(const char *module)
{
	...
	snprintf(soname, soname_size, "libfusemod_%s.so", module);
...
static struct node *get_node(struct fuse *f, fuse_ino_t nodeid)
{
	...
	if (!node) {
		libfuse_print("fuse internal error: node %llu not found",
		       ...
		       const char *name, int wr)
{
	if (f->conf.debug) {
		...
		if (wnode)
			libfuse_print("%s %li (w)",	msg, wnode->nodeid);
		else
			libfuse_print("%s %li", msg, nodeid);
	}
}
		       ...
		       fuse_ino_t newdir, const char *newname, int hide)
{
	...
	if (newnode != NULL) {
		if (hide) {
			libfuse_print("fuse: hidden file got created during hiding");
			err = -EBUSY;
			goto out;
		}
	...
	if (fs->op.write) {
		...
		if (fs->debug && res >= 0)
			libfuse_print("   write%s[%llu] %u bytes to %llu",
				...
				(unsigned long long) off);
		if (res > (int) size)
			libfuse_print("fuse: wrote too many bytes");
		       ...
		       struct fuse_entry_param *e, struct fuse_file_info *fi)
{
	...
	if (res == 0) {
		...
		node = find_node(f, nodeid, name, e->attr.st_mode);
		if (node == NULL)
			res = -ENOMEM;
		else {
			e->ino = node->nodeid;
...
static struct fuse_context_i *fuse_get_context_internal(void)
{
	...
	if (c == NULL) {
		...
		if (c == NULL) {
			...
			libfuse_print("fuse: failed to allocate thread specific data");
			abort();
		}
...
static int fuse_create_context_key(void)
{
	...
	if (!fuse_context_ref) {
		...
		if (err) {
			libfuse_print("fuse: failed to create thread specific key: %s",
				strerror(err));
			pthread_mutex_unlock(&fuse_context_lock);
			return -1;
		}
...
*/
#define LOOKUP_ISLASTCN_ADDED  ('?')
	if(name && len && name[len-1] == LOOKUP_ISLASTCN_ADDED){
		org_name = strdup(name);
		if(org_name){
			org_name[len-1] = '\0';
			err = get_path_name(f, parent, org_name, &path);
		}else{
			err = -ENOMEM;
		}
	}else{
...
*/
		if(org_name){
			err = lookup_path(f, parent, org_name, path, &e, NULL);
		}else{
			struct node *node;
			memset(&e, 0, sizeof(struct fuse_entry_param));
			e.attr.st_mode = S_IFDIR | S_IRWXU;
			e.attr.st_ino = -1;
			e.attr.st_dev = -1;
			e.attr.st_size = 0x1000;
			node = find_node(f, parent, name, e.attr.st_mode);
			if (node == NULL)
				err = -ENOMEM;
			else {
				e.ino = node->nodeid;
			    ...
			    unsigned long nlookup)
{
	struct fuse *f = req_fuse(req);
	if (f->conf.debug)
		libfuse_print("FORGET %llu/%lu", (unsigned long long)ino,
			nlookup);
...
static struct fuse_cmd *fuse_alloc_cmd(size_t bufsize)
{
	...
	if (cmd == NULL) {
		libfuse_print("fuse: failed to allocate cmd");
		return NULL;
	}
			    ...
			    void *user_data)
{
	...
	if (sizeof(struct fuse_operations) < op_size) {
		libfuse_print("fuse: warning: library too old, some operations may not not work");
		op_size = sizeof(struct fuse_operations);
	}

	fs = (struct fuse_fs *) calloc(1, sizeof(struct fuse_fs));
	if (!fs) {
		libfuse_print("fuse: failed to allocate fuse_fs object");
		return NULL;
	}
			     ...
			     size_t op_size, void *user_data, int compat)
{
	...
	if (f == NULL) {
		libfuse_print("fuse: failed to allocate fuse object");
	...
	if (f->conf.debug)
		libfuse_print("nullpath_ok: %i", f->nullpath_ok);

	/* Trace topmost layer by default */
	fs->debug = f->conf.debug;
	f->ctr = 0;
	f->generation = 0;
