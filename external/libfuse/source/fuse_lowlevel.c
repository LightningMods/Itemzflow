			       ...
			       int count)
{
	...
	if (error <= -1000 || error > 0) {
		libfuse_print("fuse: bad error value: %i",	error);
		error = -ERANGE;
	}
...
static void do_init(fuse_req_t req, fuse_ino_t nodeid, const void *inarg)
{
	...
	(void) nodeid;
	if (f->debug) {
		libfuse_print("INIT: %u.%u", arg->major, arg->minor);
		if (arg->major == 7 && arg->minor >= 6) {
			libfuse_print("flags=0x%08x", arg->flags);
			libfuse_print("max_readahead=0x%08x",
				arg->max_readahead);
		}
	}
	...
	if (arg->major < 7) {
		libfuse_print("fuse: unsupported protocol version: %u.%u",
			arg->major, arg->minor);
	...
	if (bufsize < FUSE_MIN_READ_BUFFER) {
		libfuse_print("fuse: warning: buffer size too small: %zu",
			bufsize);
	...
	if (f->debug) {
		libfuse_print("   INIT: %u.%u", outarg.major, outarg.minor);
		libfuse_print("   flags=0x%08x", outarg.flags);
		libfuse_print("   max_readahead=0x%08x",
			outarg.max_readahead);
		libfuse_print("   max_write=0x%08x", outarg.max_write);
	}
			   ...
			   int notify_code, struct iovec *iov, int count)
{
	...
	if (f->debug)
		libfuse_print("NOTIFY: code=%d count=%d length=%u",
			    ...
			    struct fuse_chan *ch)
{
	...
	if (req == NULL) {
		libfuse_print("fuse: failed to allocate request");
		return;
	}
...
{
	libfuse_print("using FUSE kernel interface version %i.%i",
			    ...
			    struct fuse_args *outargs)
{
	...
	default:
		libfuse_print("fuse: unknown option `%s'", arg);
	}
					      ...
					      size_t op_size, void *userdata)
{
	...
	if (sizeof(struct fuse_lowlevel_ops) < op_size) {
		libfuse_print("fuse: warning: library too old, some operations may not work");
