				 ...
				 int *fdp, int numfds)
{
	...
	if ((size_t) res != buflen) {
		libfuse_print("libulockmgr: sendmsg short");
		return -1;
	}
	return 0;
}
...
static struct owner *ulockmgr_new_owner(const void *id, size_t id_len)
{
	...
	if (!o) {
		libfuse_print("libulockmgr: failed to allocate memory");
		return NULL;
	}
	o->id = o + 1;
	o->id_len = id_len;
				 ...
				 size_t id_len)
{
	...
	if (res == -1) {
		...
		while (1) {
			...
			if (res != sizeof(struct message)) {
				libfuse_print("libulockmgr: send short");
				msg->error = EIO;
				break;
			}
		}
...
#ifdef DEBUG
	libfuse_print("libulockmgr: %i %i %i %lli %lli own: 0x%08x",
