		 ...
		 off_t offset, struct fuse_file_info *fi)
{
	...
	nfsdir = static_cast<struct nfsdir *>(cb_data.return_data);
	while ((nfsdirent = nfs_readdir(nfs, nfsdir)) != NULL) {
		//fuse_nfs_getattr(nfsdirent->name, &st);
		if(fuse_debug_flag)
		  log_debug("fuse_nfs_readdir: %s, d_type %i", nfsdirent->name, st.st_mode);
...
readlink_cb(int status, struct nfs_context *nfs, void *data, void *private_data)
{
	...
	strncat((char*)cb_data->return_data, (const char*)data, cb_data->max_size);
}
	      ...
	      off_t offset, struct fuse_file_info *fi)
{
	...
	if(fuse_debug_flag)
	   log_debug("fuse_nfs_read entered [%s] offset %lx", path, offset);
...
void *fuse_main_init(void *args)
{
   ...
   const char* argv[] = { 
    ...
    ret = fuse_main(fuse_args->argc, const_cast<char**>(argv), &nfs_oper, NULL);
    if (ret < 0) {
...
int fatal_fuse_error(int ret, struct nfs_url *urls )
{
...
int fuse_nfs_main(struct fuse_actions args)
{
	...
	if (nfs == NULL) {
		...
		ret = 10;
		return fatal_fuse_error(ret, urls);
	}
    ...
    struct fuse_actions *fa = new fuse_actions;
    fa->argc = args.argc;
	fa->is_debug_mode = args.is_debug_mode;
	snprintf(fa->path, sizeof(fa->path)-1, "%s", args.path);
